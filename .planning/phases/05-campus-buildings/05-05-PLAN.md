...
must_haves:
  ...
  artifacts:
    ...
    - path: "js/game/buildings.js"
      provides: "Talks interaction handler"
      contains: "showTalksContent|type.*talks"
  key_links:
...
Output: world.js with Lab, Station, Theatre interiors; sprites.js with any new interior sprites; buildings.js with talks handler.
</objective>
...
<task type="auto">
  <name>Task 3: Create TV Station and Theatre interiors with talks handler</name>
...
Add 'talks' interaction handler in Buildings.js:

1. Add to updatePrompt() switch statement:
```javascript
case 'talks':
...
2. Add to handleInteraction() switch statement:
```javascript
case 'talks':
...
3. Add showTalksContent() method:
```javascript
showTalksContent(index) {
    // Talks data could be in SITE_CONTENT.talks or reuse media
    // For now, use SITE_CONTENT.media filtered to talks-type items
    // or create a simple talks display similar to showMediaContent
...
4. Update getWelcomeMessage() to include station and theatre:
```javascript
station: "Welcome to the TV Station! Explore to see media appearances and interviews.",
theatre: "Welcome to the Lecture Theatre! Browse to discover talks and presentations."
```
...
- Can interact with talks interactions (shows talks content)
- Can exit properly

In console:
- typeof Buildings.showTalksContent === 'function'
- Buildings.handleInteraction({ type: 'talks', index: 0 }) returns { action: 'dialog' }
  </verify>
  <done>Station interior (broadcast studio) and Theatre interior (lecture hall) created with appropriate themes; talks interaction handler added to Buildings.js</done>
</task>
...
- Can interact with Theatre objects to see talks content (new handler works)
...
- Buildings.js has showTalksContent() handler for 'talks' interaction type
